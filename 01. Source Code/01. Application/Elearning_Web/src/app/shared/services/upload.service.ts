...
export class UploadService {
  ...
  // Upload mẫu template
  uploadFileTemplate(file: any, folderName: string): Observable<any> {
    let formData: FormData = new FormData();
    formData.append('folderName', folderName);
    formData.append('file', file);
    var tr = this.http.post<any>(this.config.ServerWithApiUrl + 'uploads/upload-file-template', formData);
    return tr
  }
}
